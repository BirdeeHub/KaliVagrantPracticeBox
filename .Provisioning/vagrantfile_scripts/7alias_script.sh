...
echo "function dirffufle() { echo \"\$#\"; local cnt=0; local xtraargs=(); for arg in \"\$@\"; do let \"cnt+=1\"; [[ \$cnt != 1 ]] &&  xtraargs+=(\"\$arg\"); [[ \$cnt == 1 ]] && local domain=\"\$arg\"; done; echo \"\$xtraargs\"; local full_command=\"sudo ffuf -c -w /usr/share/wordlists/dirbuster/directory-list-1.0.txt -u http://\$domain/FUZZ \$xtraargs\"; bash -c \"\$full_command\"; }" >> /home/vagrant/.zshrc