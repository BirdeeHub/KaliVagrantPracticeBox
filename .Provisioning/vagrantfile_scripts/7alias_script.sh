...
echo "function ffufbucket() {local cnt=0; local xtraargs=(); for arg in \"\$@\"; do let \"cnt+=1\"; [[ \$cnt > 2 ]] &&  xtraargs+=(\"\$arg\"); [[ \$cnt == 1 ]] && local url=\"\$arg\"; [[ \$cnt == 2 ]] && local domain=\"\$arg\"; done; local full_command=\"sudo ffuf -H \"Host: FUZZ.\$domain\" -H \"User-Agent: PENTEST\" -w \"/usr/share/wordlists/dirbuster/directory-list-1.0.txt\" -u \$url \$xtraargs\"; echo \"\$full_command\"; bash -c \"\$full_command\"; }" >> /home/vagrant/.zshrc
